function ie(t,i){const a=i?new RegExp(`struct\\s+${i}\\s*\\{([^}]+)\\}`,"s"):/struct\s+(\w+)\s*\{([^}]+)\}/s,n=t.match(a);if(!n)return[];const l=i?n[1]:n[2],r=[],f=/(\w+)\s*:\s*([\w<>]+)\s*,?/g;let o;for(;(o=f.exec(l))!==null;)r.push({name:o[1].trim(),type:o[2].trim()});return r}function ne(t,i){const a=new RegExp(`fn\\s+${i}\\s*\\([^)]*\\)\\s*->\\s*(\\w+)`,"s"),n=t.match(a);return n?n[1]:null}function ue(t,i){const{vertexShaderBody:a,fragmentShaderBody:n,format:l,vertexFunction:r="getVertex",positionField:f="position",widthField:o="width",join:u="bevel",joinResolution:w=8,maxJoinResolution:m=16,miterLimit:s=4,cap:x="round",capResolution:se=8,maxCapResolution:y=16,blend:P=null,depthFormat:E=null}=i,v=ne(a,r);if(!v)throw new Error(`Could not find vertex function '${r}' in vertexShaderBody`);const I=ie(a,v);if(I.length===0)throw new Error(`Could not parse struct '${v}' in vertexShaderBody`);const G=I.findIndex(e=>e.name===f);if(G===-1)throw new Error(`Position field '${f}' not found in struct '${v}'`);const V=I.findIndex(e=>e.name===o);if(V===-1)throw new Error(`Width field '${o}' not found in struct '${v}'. The vertex struct must include a width field.`);const S=I.filter((e,p)=>p!==G&&p!==V),b=u==="round",F=u==="bevel",R=F?0:s,W=x!=="none";let $;x==="none"?$=1:x==="square"?$=3:$=y;const A=b?m*2:2,D=$*2,T=x==="square"?[2,2/Math.sqrt(3)]:[1,1],X=(Math.max(D,A)+3)*2,K=re({userCode:a,vertexFunction:r,returnType:v,positionField:f,widthField:o,varyings:S,isRound:b}),Q=oe({userCode:n,varyings:S});S.length+1;const Z=t.createShaderModule({label:"gpu-lines-vertex",code:K}),ee=t.createShaderModule({label:"gpu-lines-fragment",code:Q});t.createBindGroupLayout({label:"gpu-lines-uniforms",entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}]});const z={label:"gpu-lines",layout:"auto",vertex:{module:Z,entryPoint:"vertexMain"},fragment:{module:ee,entryPoint:"fragmentMain",targets:[P?{format:l,blend:P}:{format:l}]},primitive:{topology:"triangle-strip",stripIndexFormat:void 0}};E&&(z.depthStencil={format:E,depthWriteEnabled:!0,depthCompare:"less"});const L=t.createRenderPipeline(z),M=t.createBuffer({label:"gpu-lines-uniforms",size:40,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),te=t.createBindGroup({layout:L.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:M}}]}),_=new ArrayBuffer(40),d=new Float32Array(_),U=new Uint32Array(_);d[2]=D,d[3]=A,d[4]=R*R,U[5]=b?1:0,U[7]=W?1:0,d[8]=T[0],d[9]=T[1];let J=-1,k=-1,q=-1,N=D,Y=A,j=R*R,H=!1;return{getBindGroupLayout(e){return L.getBindGroupLayout(e)},updateUniforms(e){const{vertexCount:p,resolution:c}=e;let B=D;if(x==="round"&&e.capResolution!==void 0){const O=Math.min(e.capResolution,y);e.capResolution>y&&console.warn(`capResolution ${e.capResolution} exceeds maxCapResolution ${y}, clamping to ${y}`),B=O*2}let g=A;if(b&&e.joinResolution!==void 0){const O=Math.min(e.joinResolution,m);e.joinResolution>m&&console.warn(`joinResolution ${e.joinResolution} exceeds maxJoinResolution ${m}, clamping to ${m}`),g=O*2}let C=j;!F&&e.miterLimit!==void 0&&(C=e.miterLimit*e.miterLimit),(!H||p!==J||c[0]!==k||c[1]!==q||B!==N||g!==Y||C!==j)&&(d[0]=c[0],d[1]=c[1],d[2]=B,d[3]=g,d[4]=C,U[6]=p,t.queue.writeBuffer(M,0,_),J=p,k=c[0],q=c[1],N=B,Y=g,j=C,H=!0)},draw(e,p,c=[]){const{vertexCount:B,skipUniformUpdate:g}=p;g||this.updateUniforms(p);const C=Math.max(0,B-1);if(C>0){e.setPipeline(L),e.setBindGroup(0,te);for(let h=0;h<c.length;h++)e.setBindGroup(h+1,c[h]);e.draw(X,C)}},destroy(){M.destroy()}}}function re({userCode:t,vertexFunction:i,returnType:a,positionField:n,widthField:l,varyings:r,isRound:f}){const o=r.map((s,x)=>`  @location(${x+1}) ${s.name}: ${s.type},`).join(`
...
`}function oe({userCode:t,varyings:i}){const a=i.map((u,w)=>`  @location(${w+1}) ${u.name}: ${u.type},`).join(`
